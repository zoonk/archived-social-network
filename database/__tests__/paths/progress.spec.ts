...
beforeAll(async (done) => {
  ...
  collection = db.collection('paths/newItem/progress');
...
test('cannot update', async (done) => {
  await admin.doc('paths/newItem/progress/currentUser').set({ posts: 0 });
...
test('users cannot read data from others', async (done) => {
  const ref = db.doc('paths/newItem/progress/otherUser');
