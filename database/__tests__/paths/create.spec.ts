...
test('level has a valid string', async (done) => {
  await firebase.assertSucceeds(ref.add({ ...data, level: 'advanced' }));
  await firebase.assertSucceeds(ref.add({ ...data, level: 'pro' }));
  await firebase.assertSucceeds(ref.add({ ...data, level: 'expert' }));
  await firebase.assertFails(ref.add({ ...data, level: 'other' }));
  done();
});
