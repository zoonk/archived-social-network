...
beforeAll(async (done) => {
  ...
  collection = db.collection('replies/newItem/likes');
...
test('cannot read data from other users', async (done) => {
  const ref = db.doc('replies/newItem/likes/otherUser');
...
test('cannot update data from other users', async (done) => {
  ...
  await admin.doc('replies/newItem/likes/otherUser').set({ like: true });
