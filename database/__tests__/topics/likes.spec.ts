...
beforeAll(async (done) => {
  ...
  collection = db.collection('topics/newItem/likes');
...
test('cannot read data from other users', async (done) => {
  const ref = db.doc('topics/newItem/likes/otherUser');
...
test('cannot update data from other users', async (done) => {
  ...
  await admin.doc('topics/newItem/likes/otherUser').set({ like: true });
