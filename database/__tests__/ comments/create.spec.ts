...
test('content field has at least 1 character', async (done) => {
  await firebase.assertFails(ref.add({ ...data, content: '' }));
  await firebase.assertSucceeds(ref.add({ ...data, content: '👍' }));
  done();
});
