...
beforeAll(async (done) => {
  ...
  collection = db.collection('comments/newItem/likes');
...
test('cannot read data from other users', async (done) => {
  const ref = db.doc('comments/newItem/likes/otherUser');
...
test('cannot update data from other users', async (done) => {
  ...
  await admin.doc('comments/newItem/likes/otherUser').set({ like: true });
