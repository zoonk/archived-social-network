...
export const getStaticProps: GetStaticProps<GroupBooksProps> = async ({
  params,
}) => {
  const groupId = String(params?.id);
  const groupReq = getGroup(groupId);
  const booksReq = getPosts({ category: ['books'], groupId, limit });
  const [group, books] = await Promise.all([groupReq, booksReq]);
  return { props: { books, group }, unstable_revalidate: 1 };
};
  ...
  return (
    <Container component="main">
      <Meta
        ...
        image={photo}
        canonicalUrl={`https://${language}.zoonk.org/groups/${id}/books`}
      />
      <GroupsBreadcrumb
        groupId={id}
        groupName={title}
      ...
      <GroupBase group={group}>
        <PostShare
          ...
          groupId={id}
        />
        <PostsList data={books} category={['books']} groupId={id} limit={10} />
      </GroupBase>
    </Container>
  );
};
