...
import { onCreateDocUpdateStats } from '../updateStats';
...
test('do not update untracked collections', async (done) => {
  ...
  const wrapped = testEnv.wrap(onCreateDocUpdateStats);
...
test('update the posts count', async (done) => {
  ...
  expect(req).toBe('updated');
  expect(db.doc).toHaveBeenCalledWith('admin/stats');
  expect(db.doc('').update).toHaveBeenCalledWith({ examples: 1, timeline: 1 });
  done();
});
