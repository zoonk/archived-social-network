...
import { onCreatePostUpdateCount } from '../updateCount';

test('return when there is no chapterId', async (done) => {
  const snap = { data: () => ({ chapterId: null }) };
  const wrapped = testEnv.wrap(onCreatePostUpdateCount);
...
test('increment the count for examples', async (done) => {
...
test('increment the count for lessons', async (done) => {
  spyOn(db.doc(''), 'update').and.returnValue('updated');
...
test('do not increment the count for other categories', async (done) => {
  spyOn(db.doc(''), 'update').and.returnValue('updated');
